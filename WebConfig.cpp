...
"function getCaption(vname,vvalue) {\n"
"var chan = [%s];\n"
"var moni = [%s];\n"
"var fx = [%s];\n"
" var valRet='('+vvalue+')';\n"
" if (vname.includes('channel')) valRet+=chan[vvalue-1];\n"
" if (vname.includes('monitor')) valRet+=moni[vvalue-1];\n"
" if (vname.includes('fx')) valRet+=fx[vvalue-1];\n"
" return valRet;\n"
"}\n"
"function writeCaption(vname,vvalue) {\n"
"  document.write(getCaption(vname,vvalue));\n"
"}\n"
"</script>\n"
"</head>\n"
"<body>\n"
...
"  <div class='zeile'>%s: <b><output name='val%s'><script>writeCaption('%s',%s)</script></output></b></div>\n"
"  <div class='zeile'><input type='range' min='%i' max='%i' value='%s' name='%s' oninput='val%s.value=getCaption(\"%s\",%s.value)'/></div>\n";
...
void createRange(char * buf, DESCRIPTION descr, String value) {
  sprintf(buf,HTML_ENTRY_RANGE,descr.label,descr.name,descr.name,value.c_str(),descr.min,descr.max,value.c_str(),descr.name,descr.name,descr.name,descr.name);
}
...
//******************** Rest of the function has no difference ***************
  ...
  sprintf(_buf,HTML_START,
	chNames.c_str(),
	moniNames.c_str(),
	fxNames.c_str(),
    _apName.c_str());
  server->send(200, "text/html", _buf);
  createSimple(_buf,"apName","Device/AP Name","text",_apName);
  ...
  if (server->args() > 0) {
    ...
    if (server->hasArg("SAVE") || server->hasArg("RST")) {
	  ...
	  server->sendContent("<script>alert('CONFIGURATION SAVED');</script>");
